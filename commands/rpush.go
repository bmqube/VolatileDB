...
func (cmd *RPushCommand) Execute(args []models.Message, store *store.Storage, mb *resp.MessageBuilder) models.Message {
	...
	length, err := store.RPush(string(key.BulkString), values)
...
func (cmd *RPushCommand) ValidateArgs(args []models.Message) error {
	if len(args) < 2 {
